"""Streamlit portfolio demo — Premium Auto CPO Retail Command Center.
...
deterministic. No real company data, dealer names, customers, VINs, targets, or
...
FOCUS_DEALER_ID = "DLR-E07"        # the dealer that drops in ranking (demo story)
CPO_MODELS = ["Compact Sedan", "Mid Sedan", "Executive Sedan", "Luxury Sedan", "Compact SUV", "Mid SUV", "Large SUV", "Electric Sedan"]
CPO_MODELS_ZH = {
    "Compact Sedan": "紧凑轿车", "Mid Sedan": "中型轿车", "Executive Sedan": "行政轿车", "Luxury Sedan": "豪华轿车",
    "Compact SUV": "紧凑SUV", "Mid SUV": "中型SUV", "Large SUV": "大型SUV", "Electric Sedan": "纯电轿车",
}


@st.cache_data(show_spinner=False)
def generate_dealer_network() -> Dict[str, pd.DataFrame]:
    """Deterministic synthetic premium-auto dealer network (illustrative only)."""
    rng = np.random.default_rng(2026)
    rows = []
    for r_i, region in enumerate(REGIONS):
        for d in range(DEALERS_PER_REGION):
            did = f"DLR-{region[0]}{d + 1:02d}"
    ...
    # ── CPO inventory for the focus dealer (Executive Sedan overstocked & overpriced) ──
    inv_rng = np.random.default_rng(77)
    base_value = {"Compact Sedan": 195_000, "Mid Sedan": 285_000, "Executive Sedan": 405_000,
                  "Luxury Sedan": 720_000, "Compact SUV": 235_000, "Mid SUV": 345_000,
                  "Large SUV": 525_000, "Electric Sedan": 480_000}
    weights = np.array([0.08, 0.16, 0.34, 0.05, 0.10, 0.14, 0.08, 0.05])  # Executive Sedan heavy
    inv = []
    for i in range(140):
        ...
        # Executive Sedan skews older stock; everything else fresher
        days_in_stock = int(np.clip(inv_rng.normal(95 if model == "Executive Sedan" else 46, 28), 5, 190))
        fair = base_value[model] * max(0.5, 1 - 0.09 * age) * max(0.7, 1 - 0.012 * km)
        fair *= inv_rng.normal(1.0, 0.03)
        # overpricing concentrated in aged Executive Sedan
        if model == "Executive Sedan" and days_in_stock >= 75:
...
with exec_tab:
    ...
    st.markdown(f'<div class="alert-box">⚠️ {t("Alert: national sales on track, but dealer <b>DLR-E07</b> dropped to rank "+str(int(dealers.loc[dealers.dealer_id==FOCUS_DEALER_ID,"rank"].iloc[0]))+" of 50. Drill into Sales → After-sales → Dealer 360 → CPO to see why.","预警：全国销量达成正常，但经销商 <b>DLR-E07</b> 综合排名跌至第 "+str(int(dealers.loc[dealers.dealer_id==FOCUS_DEALER_ID,"rank"].iloc[0]))+"/50。沿 Sales → After-sales → Dealer 360 → CPO 下钻查看原因。")}</div>',
                unsafe_allow_html=True)
...
with cpo_tab:
    ...
    st.markdown(f'<div class="story-box">{t("Combined action plan for DLR-E07","DLR-E07 组合行动建议")}: '
...
with ai_tab:
    ...
    questions_en = [
        "Why did DLR-E07 drop in the ranking?",
    ...
    answers_en = [
        ...
        f"The {weak_region} region has the lowest CPO achievement, pulled down by DLR-E07's {focus.cpo_achv:.0%}. NC and Vans are on track there, which is why the national number still looks healthy.",
        f"Service retention and CSI at DLR-E07 ({focus.service_retention:.0%} / {focus.csi:.0f}) are the clearest after-sales risk. Customers who bought there are not coming back for service, which compounds the CPO inventory problem.",
        f"{len(aged)} CPO units are 90+ days in stock, concentrated in Executive Sedan. Reprice the overpriced ones toward model fair value, promote the rest, and transfer slow movers to higher-demand stores. Estimated margin at risk is meaningful but recoverable with early action.",
    ]
    answers_zh = [
        f"DLR-E07 排名第 {int(focus['rank'])}/50。NC 达成正常（{focus.nc_achv:.0%}），但 CPO 仅 {focus.cpo_achv:.0%}，库存天数 {focus.days_supply:.0f}，90 天以上库存率 {focus.aging_90:.0%}。售后进一步拖累：回厂率 {focus.service_retention:.0%}、满意度 {focus.csi:.0f}。在 45/35/15/5 的综合权重下，这两个缺口直接拉低了综合排名。",
        f"{REGIONS_ZH.get(weak_region, weak_region)}区 CPO 达成最弱，主要被 DLR-E07 的 {focus.cpo_achv:.0%} 拉低。该区 NC 与 Vans 正常，所以全国数字看起来仍健康。",
...
<b>premium automotive dealer data analytics</b> with a used-car dynamic-pricing model trained on a public
dataset, integrated into one retail decision system.<br><br>
<b>What I did in dealer analytics → so I designed it this way:</b><br>
...
""".strip(), language="text")
    ...
    if lang == "中文":
        st.markdown("""
本作品集**不使用任何真实企业数据**。所有经销商网络数字、目标、客户、车辆、VIN 均为合成且确定性生成，
仅用于演示指标体系与决策闭环。Dealer Score 权重、车型组合与 KPI 公式是透明的作品集设计假设，
**不代表任何企业的官方定义**。车型名称（行政轿车、SUV 等）仅为示意分段。单车定价模型基于公开二手车数据集，
公开应用使用规则模拟器以保持仓库轻量。
""")
    else:
        st.markdown("""
This portfolio uses **no real company data**. All dealer-network figures, targets, customers,
...
assumptions, **not any company's official definitions**. Segment labels (Executive Sedan, SUVs, etc.) are illustrative.
